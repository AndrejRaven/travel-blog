...
interface KursyWalutClientProps {
  slug: string;
  embedded?: boolean;
}

export default function KursyWalutClient({ slug, embedded = false }: KursyWalutClientProps) {
  const { trip, isLoading, status } = useTripData(slug);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      setSelectedPairs((items) => {
        ...
        saveSelectedPairs(slug, newOrder);
        return newOrder;
      });
    }
  };
  ...
  // Pobierz listę wszystkich dostępnych walut (z cache - raz na godzinę)
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.currencies) {
            const currencyCodes = (data.currencies as { code: string }[]).map((c) => c.code);
            ...
            const filteredCurrencies: string[] = uniqueCurrencies.filter((code, index) => 
              code !== "PLN" || uniqueCurrencies.indexOf("PLN") === index
            );
            setAvailableCurrencies(filteredCurrencies);
            // Zapisz do cache
            setCachedCurrencies(filteredCurrencies, data.date);
        ...
        // Jeśli błąd, użyj cache nawet jeśli wygasł, lub fallback
        if (cached) {
          setAvailableCurrencies(cached);
        } else {
  ...
  const handleAddPair = () => {
    if (selectedFromCurrency && selectedToCurrency && selectedFromCurrency !== selectedToCurrency) {
      ...
      if (!exists) {
        ...
        saveSelectedPairs(slug, [...selectedPairs, pair]);
  ...
  if (isLoading) {
    ...
    return embedded ? loadingMsg : <PageLayout maxWidth="4xl">{loadingMsg}</PageLayout>;
  }

  if (!trip) {
    const noTripMsg =
      status === "offlineIndexOnly" ? (
        <div className="text-center py-12">
          <p className="text-gray-700 dark:text-gray-300 mb-3">
            Kursy walut dla tej podróży nie są dostępne offline.
          </p>
          <p className="text-gray-600 dark:text-gray-400">
            Połącz się z internetem, aby pobrać aktualne kursy.
          </p>
          {!embedded && <BackToHome className="mt-6" />}
        </div>
      ) : (
        <div className="text-center py-12">
          <p className="text-gray-600 dark:text-gray-400">
    ...
    return embedded ? noTripMsg : <PageLayout maxWidth="4xl">{noTripMsg}</PageLayout>;
  }
