...
import { History, Plus, ArrowRight, Filter, Calendar, ArrowUpDown, X, Edit, Trash2 } from "lucide-react";
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2 text-base font-medium text-gray-900 dark:text-gray-100">
                <span>{formatCurrency(tx.fromAmount, tx.fromCurrency, 2)}</span>
                <ArrowRight className="w-4 h-4 text-gray-400" />
                <span>{formatCurrency(tx.toAmount, tx.toCurrency, 2)}</span>
              </div>
              {tx.fee && tx.feeCurrency && (
                <div className="text-xs text-red-600 dark:text-red-400 mt-1">
                  Prowizja: {formatCurrency(tx.fee, tx.feeCurrency, 2)}
                </div>
              )}
              {tx.location && (
