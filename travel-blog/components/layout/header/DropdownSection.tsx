  ...
  return (
    <div className="px-2 py-2">
      ...
      >
        ...
        <ChevronIcon
          className={`h-4 w-4 transition-transform duration-300 text-gray-600 ${
      ...
      <ul
        className={`${
          isOpen ? "block" : "hidden"
        } mt-2 space-y-1 text-sm text-gray-700 overflow-hidden`}
      >
        {section.items.map((item) => (
          <li key={item.href}>
            <a
              href={item.href}
              className="flex items-center gap-2 rounded-md px-2 py-1.5 hover:bg-gray-50 hover:text-gray-900 transition-colors min-w-0 group/item"
            >
              <CircleIcon
                className="h-3.5 w-3.5 text-gray-400 group-hover/item:text-gray-600 flex-shrink-0 transition-colors"
                aria-hidden
              />
